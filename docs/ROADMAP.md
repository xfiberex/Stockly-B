...
> ## Estado al 2026-09-13 — **118 / 129**
>
> **Se abre el [Tier 5](#tier-5--funcionalidad-de-negocio), con 15 tareas, y se cierran las cuatro primeras:** T5-01,
> coste medio ponderado —con la cancelación de compras recibidas llevada a la interfaz—; T5-02, valor
> del inventario a coste y margen realizado en dashboard, informe y PDF; T5-03, stock comprometido y
> disponible, con las ventas que lo superan bloqueadas; y T5-04, recepción parcial de compras. No
...
- [x] **[T4-09] Auditoría de navegador y de lector de pantalla** ✅ *(2026-08-11, **con una salvedad**: ver la última nota)*
  - **Área:** Accesibilidad / Rendimiento
  - **Ubicación:** aplicación desplegada, [`docs/accesibilidad.md`](accesibilidad.md), `Stockly-F/src/tests/accesibilidad.test.ts`, `Stockly-F/src/shared/lib/clasesDeMarco.ts`
  ...
  - **Auditado sobre la pila del compose**, no sobre `vite dev`: build de producción tras nginx, que es lo que pedía la ficha. `vite preview` tampoco valía de sustituto —no aplica `server.proxy` y la SPA se quedaría sin API—.
  - **El criterio no se cumplía: el login estaba en 93.** Y es la primera pantalla del producto. Las cuatro pantallas auditadas quedan en **100** (login 93, productos 94, movimientos 95, dashboard ya en 100).
  - **Tres fallos, y ninguno se notaba mirando la pantalla.** **Ocho desplegables sin nombre accesible** en cuatro pantallas —el texto visible es la opción elegida, que dice el *valor* y no de qué es el filtro—; **las siete pantallas sin sesión sin landmark `<main>`**, porque no pasan por `App.tsx` y llevaban el marco copiado; y **enlaces distinguidos solo por el color** en login y registro, con el subrayado reservado al `hover` (WCAG 1.4.1, la misma regla por la que aquí ningún estado se comunica solo con color).
  - **Un cuarto que Lighthouse no marca**, encontrado leyendo el árbol de accesibilidad: el menú de usuario enseñaba «Admin Principal» y se anunciaba «Menú de usuario» (WCAG 2.5.3). Quien maneja el ordenador **por voz** dice lo que ve y no pasaba nada.
  - **Los dos flujos, recorridos sin ratón.** El de movimiento de stock **ejecutado de verdad**: el stock pasó de 10 a 15. La trampa de foco del modal se comprobó **por sus dos bordes**, que es donde se rompe —`Tab` en el último elemento y `Shift+Tab` en el primero—, y `Escape` devuelve el foco al disparador.
...
- [x] **[T4-17] Recorrido real con lector de pantalla** — **descartada** *(2026-08-12)*
  ...
  - **Qué hacer:** T4-09 dejó las cuatro pantallas principales en 100 de Lighthouse y verificó que los flujos de alta de producto y movimiento de stock se completan **solo con teclado**, leyendo además el árbol de accesibilidad, que es la fuente de lo que anuncia un lector. Lo que no se hizo es escucharlo: la máquina de trabajo no tiene NVDA ni VoiceOver. Y no es lo mismo — el árbol dice **qué** se anunciaría, no si la secuencia se entiende de oído: un orden razonable en el DOM puede resultar incomprensible leído, los avisos de las notificaciones pueden no llegar a anunciarse y una tabla puede recitar sus cabeceras en cada celda. Recorrer los dos flujos con NVDA (Windows) o VoiceOver (macOS) y anotar lo que suene mal.
...
- [x] **[T5-02] Valor del inventario a coste y margen en los informes** ✅ *(2026-09-13)*
  ...
  - **Esfuerzo:** medio
  - **Depende de:** T5-01
  - **Verificado localmente (2026-09-13):** `verify` backend con **517/517** tests (13 nuevos en `margen.test.ts`), sentencias **93.8 %**, `check`/`build`/`smoke` en verde y **`auditoria` en rojo por los mismos 10 avisos ajenos** que ya anotó T5-01. `verify` frontend en verde, **564/564 + 1 omitido**, sentencias **70.49 %**.
  - **Los dos criterios, medidos.** (1) El test del criterio envía una venta de 3 a 200 con coste 120, comprueba margen 240 y **cambia después el coste a 190**: el margen sigue en 240, donde con el coste actual caería a 30. (2) Sobre `Stockly_carga` —100 000 productos, 330 000 órdenes y 660 000 ítems sembrados para la ocasión— **ninguna de las cuatro consultas ordena en disco**: totales 47.9 ms, margen total 74.3, por categoría 149.6 y top de productos 187.3. Tabla y planes en [rendimiento.md §7 bis](rendimiento.md).
  - **En la aplicación levantada, por la interfaz:** venta de 2 unidades del producto de prueba de T5-01 (precio 20, coste 4) creada y enviada desde la pantalla de ventas → `/reports` devuelve **margen 32, 80 %**, con el ítem congelado a coste 4. El dashboard enseña valor a coste, a precio de venta y margen potencial, y avisa de los **46 productos con stock sin coste** de la base de desarrollo, que es anterior a T5-01. Revisado también a 412 px, sin desbordamiento horizontal.
  - **`SaleOrder` no guardaba cuándo se envió, y la ficha lo daba por hecho.** Pedía el margen de un periodo sin reparar en que `updatedAt` cambia con cualquier edición. Se añade `shippedAt`, con índice `(status, shippedAt)`; la migración rellena las ventas ya enviadas con su `updatedAt`, que es la mejor aproximación que queda y está escrita como tal. Se conserva si la orden se cancela después: el estado ya dice que no cuenta.
  - **Decisiones de la tarea:** la ventana es de **30 días**, la misma que la rotación, para que las dos tablas del informe hablen del mismo mes (los rangos a elegir son T5-09). El margen potencial se calcula **solo sobre los productos con coste**: restar el coste de unos al precio de todos lo inflaría. El top de productos ordena por margen **en importe**, no en porcentaje. Lo vendido sin coste no suma como coste cero —daría un 100 %—: queda fuera y se informa en `revenueWithoutCost`, en la pantalla y en el PDF.
  - **`unitCost` va en `Decimal(12, 4)`**, como el coste del producto, y se lee **de la fila ya bloqueada** por el descuento de stock, no del `include` de antes: entre las dos lecturas una recepción podría haber cambiado el coste medio.
  - **Un error de concurrencia de latencia, corregido antes de medir el endpoint:** la primera versión esperaba las tres consultas de margen **después** de las demás, sumando sus ~190 ms a la primera pantalla tras el login. Ahora arrancan a la vez. **No se ha repetido `pnpm carga:ejecutar`**, así que el p(95) de 337 ms de T4-16 no está vuelto a medir con ellas.
  - **Un error propio de la medición, visto en el plan:** la primera siembra asignaba el producto de cada ítem con `random()` dentro de la condición del `JOIN`; el planificador lo evaluó una vez y las 660 000 líneas apuntaban al mismo producto. Las consultas salían rapidísimas por agrupar un solo grupo. Detalle en rendimiento.md.
  - **Destapó una carencia de i18n anterior:** el desglose de **stock** por categoría manda desde el servidor el literal «Sin categoría», que en inglés se ve en español. El de margen **no la repite**: manda `name: null` y la interfaz lo rotula en su idioma. Corregir el de stock es cambiar un contrato ya publicado y queda fuera.
  - **Falsificados:** sumar las ventas sin coste o calcular el margen potencial sobre todos los productos tumba **2** tests del backend; pintar `null` como nombre de categoría o esconder el aviso de productos sin coste tumba **2** del frontend.
  - **El seed** congela en las ventas enviadas el coste que calcula T5-01 y les pone fecha de envío; de sus 5 ítems enviados, 3 tienen coste y 2 no, así que el caso «fuera del cálculo» también se ve con los datos de demostración. Ejecutado contra `Stockly_test`.
  - **Pendiente, anotado:** `load/sembrar.js` no genera ventas, así que rehacer `Stockly_carga` borra las que se sembraron a mano y la tabla de rendimiento no se puede repetir con el guion versionado. El PDF se comprueba por test (responde un `%PDF-` válido con la sección de margen), pero **no se ha abierto a mirar**.

### Ventas y compras

- [x] **[T5-03] Stock comprometido y stock disponible** ✅ *(2026-09-13)*
  - **Área:** Negocio
  ...
  - **Esfuerzo:** medio
  - **Depende de:** —
  - **Decisión tomada (2026-09-13): avisar y no permitir.** El formulario enseña el disponible al elegir el producto, lo marca al pasarse y no deja enviar hasta bajar la cantidad; el backend rechaza igualmente con **409 `INSUFFICIENT_AVAILABLE_STOCK`**, que queda como red para cuando el disponible cambió entre abrir el formulario y guardar. Sin ajuste en Configuración: un solo camino.
  - **Verificado localmente (2026-09-13):** `verify` backend con **530/530** tests (13 nuevos en `disponible.test.ts`), sentencias **94.09 %**, `check`/`build`/`smoke` en verde y **`auditoria` en rojo por los 10 avisos ajenos** de siempre. `verify` frontend en verde, **572/572 + 1 omitido**, sentencias **72.06 %**.
  - **El criterio, en la aplicación levantada y por la interfaz:** producto con stock 8 → venta pendiente de 5 → `/products` da **comprometido 5, disponible 3**; nueva venta de 4 → **«Supera lo disponible» y no se crea**; bajada a 3, el error desaparece (se cerró sin guardar para no dejar otra venta). El catálogo enseña «3 disp.» bajo el stock y el detalle, «5 unidades comprometidas en ventas pendientes». Enviar, cancelar y borrar una pendiente liberan el comprometido: cubierto por test.
  - **Lo que cuenta:** ventas `PENDING`, solo ítems ligados a un producto, y **el mismo producto en varias líneas suma** —dos líneas de 3 sobre 5 disponibles no caben aunque cada una sí—, tanto en el backend como en el formulario.
  - **Concurrencia, y un test que no probaba nada.** Dos ventas simultáneas de las mismas unidades leerían el mismo disponible; el servicio bloquea las filas de los productos con `FOR UPDATE`, ordenadas por id, antes de contar. **El primer test —un `Promise.all` de dos peticiones— seguía en verde con el bloqueo quitado**, tres de tres veces: las peticiones no llegaban a solaparse en la base. Se reescribió forzando la carrera con una transacción del propio test que retiene la fila; ahora, sin el bloqueo, cae con 201 donde espera 409.
  - **Tests anteriores que chocaban con la decisión:** dos de `sale-orders.test.ts` creaban por API una venta mayor que el stock para probar que **el envío** falla. Eso ya no se puede crear, pero el caso sigue existiendo —el stock puede bajar después por un ajuste— y se reescribieron así, conservando lo que vigilan.
  - **La rotación cuenta los días sobre el disponible**, como pedía la nota de la ficha: lo comprometido ya tiene dueño y contarlo como cobertura retrasaba el aviso de reponer. La tabla del informe y el PDF enseñan la columna «Disponible».
  - **Contrato:** `productoConDisponibleSchema` —`committedStock` y `availableStock`— solo en `GET /products` y `GET /products/:id`, como tipo aparte (`ProductWithAvailability` en el frontend) y no como campo opcional de `Product`, para que no se pueda leer donde no viene. **El disponible puede ser negativo** con datos anteriores a la tarea y se enseña tal cual; el mensaje de error lo recorta a 0. Un `productId` inexistente responde ahora **404** en vez de reventar en la clave foránea.
  - **Destapó que la caché de productos no se refrescaba tras las órdenes:** crear, enviar o cancelar una venta y recibir una compra solo invalidaban su propia lista, así que el stock —y ahora el disponible— se quedaba viejo en pantalla hasta recargar. Las seis mutaciones invalidan también la de productos.
  - **Falsificados:** quitar el `FOR UPDATE` o comprobar cada línea por separado tumba un test del backend cada uno; anular la validación del formulario tumba **3** del frontend.
  - **E2E: 9/9 + 1 omitido, en escritorio y móvil.** El escenario de la venta cancelada gana un paso: con 17 disponibles, pedir 18 muestra «Supera lo disponible» y no crea la orden. Al cerrar la tarea quedó sin lanzar porque `e2e/global-setup.ts` resiembra la base de desarrollo; el usuario lo autorizó después y se ejecutó con copia previa (`db:backup`). **Contra sus servidores levantados falló 7 de 9 por el límite de peticiones** —el backend arrancado a mano tiene el techo por defecto, 100 cada 15 min, y `reuseExistingServer` no aplica el `RATE_LIMIT_MAX` de `playwright.config.ts`— y se lanzó contra un backend aparte en el 3100 y un Vite en el 5174. **Destapó que T5-02 había roto el smoke:** buscaba la tarjeta «Valor total del inventario activo», que T5-02 partió en valor a coste y a precio de venta; corregido en `e2e/smoke.spec.ts`.
  - **Fuera de alcance:** el formulario de venta solo carga **200 productos activos** —límite anterior a la tarea—; un producto fuera de esos 200 no enseña disponible en el formulario, aunque el backend lo sigue comprobando. El recuento de stock bajo y la alerta por correo siguen mirando el **stock físico**, no el disponible.

- [x] **[T5-04] Recepción parcial de órdenes de compra** ✅ *(2026-09-13)*
  - **Área:** Negocio
  ...
  - **Esfuerzo:** alto
  - **Depende de:** —
  - **Verificado localmente (2026-09-13):** `verify` backend con **547/547** tests (17 nuevos en `recepcion-parcial.test.ts`), sentencias **94.23 %**, `check`/`build`/`smoke` en verde y **`auditoria` en rojo por los 10 avisos ajenos** de siempre. `verify` frontend en verde, **582/582 + 1 omitido** (10 nuevos en `RecepcionParcial.test.tsx`), sentencias **72.27 %**. **E2E 11/11 + 1 omitido**, con un escenario nuevo en escritorio y móvil.
  - **El criterio, cada punto con su test:** 60 de 100 → stock +60 y `PARTIALLY_RECEIVED`; los 40 restantes → `RECEIVED`; 41 cuando quedan 40 → **400 `RECEIPT_EXCEEDS_PENDING`** con `{ producto, pendiente, requerido }` y nada cambia; cancelar la parcial → retira 60 y deja el stock donde estaba. **Los seis tests de cancelación de T0-05 pasan sin tocarlos.**
  - **La migración, probada con datos y no solo en vacío:** en una base temporal se aplicó la cadena hasta T5-02, se metieron órdenes `RECEIVED`, `PENDING` y `CANCELLED`, y después T5-04: solo las líneas de las recibidas quedan con `receivedQuantity = quantity`. Las canceladas se dejan en 0 a propósito —su stock ya se retiró—. El valor nuevo del enum va `AFTER 'PENDING'`, que Prisma no pone y deja el orden de la base igual al del esquema.
  - **En la app levantada, por la interfaz:** la orden a medias del seed (20 de 51) → diálogo con lo que falta de cada línea; 4 donde quedan 3 → «Solo quedan 3» y el botón desactivado; 3 y 5 → «28 de 51 uds. recibidas», stock +3 y +5 con un movimiento `IN` cada uno, el coste medio del producto que no lo tenía tomado de esta entrega, y la auditoría con lo recibido.
  - **Diseño:** `POST /purchase-orders/:id/receipts` con `{ items: [{ itemId, quantity }] }`; las líneas que no van no reciben nada, y una misma línea dos veces es 422 —sumarían y esquivarían el tope—. `PATCH status: RECEIVED` se conserva como «recibir lo que falta». `PARTIALLY_RECEIVED` no se puede escribir por `PATCH`: se llega recibiendo. Las líneas escritas a mano cuentan para cerrar la orden pero no mueven inventario. El coste medio (T5-01) promedia **lo recibido en cada entrega**, no lo pedido.
  - **Concurrencia, y un hueco que ya existía:** el estado se leía **fuera** de la transacción, así que dos «recibir» simultáneos de la misma orden podían sumar el stock dos veces desde antes de esta tarea. Recepción, `PATCH` y cancelación bloquean ahora la orden con `FOR UPDATE` y leen las líneas después. Test con la carrera forzada, igual que en T5-03: sin el bloqueo, cae.
  - **Cambios de comportamiento que no pedía la ficha:** una orden con mercancía recibida **ya no vuelve a `PENDING`** (400 `CANNOT_REOPEN_RECEIVED_ORDER`) —antes se aceptaba y dejaba ese stock sin orden que lo explicara, y recibible otra vez—, y una a medias **no se elimina**, como una recibida. Nadie los usaba desde la interfaz.
  - **Falsificados:** cancelar con `quantity` tumba 3 tests del backend; quitar el `FOR UPDATE` y que `PATCH RECEIVED` sume lo pedido, uno cada uno. En el frontend, retirar lo pedido en el diálogo, quitar el tope y mandar líneas a 0 tumban 1, 1 y 3.
  - **Lo que enseñó el navegador:** el diálogo de recepción, con el ancho por defecto, **dejaba fuera la columna del campo**: la tabla compartida pide 640 px y el modal daba 624. Se abrió a `max-w-3xl`. Ningún test de jsdom lo ve.
  - **E2E:** escenario nuevo —60 de 100 por la interfaz, «60 de 100 uds. recibidas» en la tarjeta, cancelar retira 60—. Falló la primera vez **por los dos proyectos en paralelo**: cada uno crea una orden igual y el texto salía dos veces; se acota a la tarjeta de su orden, como ya hacía el escenario de ventas.
  - **Tropiezo del entorno:** un segundo `migrate dev --create-only` sobre una migración aún sin editar **creó otra vacía y aplicó las dos** a la base de desarrollo. Se borró la vacía, se quitó su fila de `_prisma_migrations` y se actualizó el checksum de la editada; `migrate reset` pide consentimiento explícito del usuario y no se forzó. `migrate status` y `migrate diff` quedan limpios. En desarrollo el valor del enum quedó al final (antes de añadir `AFTER`), sin efecto funcional.
  - **El seed** siembra una orden `PARTIALLY_RECEIVED` («MegaSupply Corp», 20 de 51) con su libro mayor y su coste medio calculados sobre lo recibido.
  - **Fuera de alcance:** la exportación CSV de compras no lleva `receivedQuantity`; la auditoría rotula igual «Orden recibida» una entrega parcial que la última (el detalle dice el estado); no hay historial por entrega más allá de los movimientos `IN` y la auditoría.
...
|---|---|---|---|
| 2026-09-13 | **T5-04** Recepción parcial de órdenes de compra — **completada** | **El criterio con tests:** 60 de 100 → parcial; 40 más → `RECEIVED`; pasarse → 400 `RECEIPT_EXCEEDS_PENDING`; cancelar la parcial retira 60. **T0-05 intacto y en verde.** Migración probada con órdenes reales en una base temporal. **Por la interfaz de la app levantada:** entrega 3 + 5 sobre la parcial del seed, con el tope visible. `verify` backend **547/547**, **94.23 %**, `auditoria` ❌ por los 10 avisos ajenos; frontend ✅ **582/582 + 1 omitido**, **72.27 %**; **E2E 11/11 + 1 omitido** | `POST /purchase-orders/:id/receipts` por líneas; `PATCH RECEIVED` recibe lo que falta. **Hueco previo cerrado:** el estado se leía fuera de la transacción y dos recepciones simultáneas sumaban dos veces; ahora `FOR UPDATE` sobre la orden. Una orden con mercancía ya no vuelve a pendiente ni se elimina a medias. **El navegador destapó** que el diálogo cortaba la columna del campo (tabla de 640 px en un modal de 624). Tropiezo: un segundo `migrate dev --create-only` aplicó una migración vacía; reparado sin `reset`. Fuera: CSV sin `receivedQuantity`. |
| 2026-09-13 | **T5-03** Stock comprometido y stock disponible — **completada** | **El criterio, por la interfaz de la app levantada:** stock 8 + venta pendiente de 5 → **disponible 3**; otra de 4 → **«Supera lo disponible», no se crea**. Por API: 409 `INSUFFICIENT_AVAILABLE_STOCK`; enviar, cancelar o borrar la pendiente libera el comprometido. `verify` backend **530/530**, sentencias **94.09 %**, `auditoria` ❌ por los 10 avisos ajenos; frontend ✅ **572/572 + 1 omitido**, **72.06 %**. E2E **9/9 + 1 omitido** | **Decisión:** avisar y no permitir, sin ajuste. El mismo producto en varias líneas suma. **Concurrencia:** `FOR UPDATE` sobre los productos antes de contar; **el primer test de carrera no probaba nada** —en verde sin el bloqueo, 3 de 3— y se rehízo reteniendo la fila desde el test. Rotación sobre el disponible. Contrato: tipo aparte solo en `GET /products` y `/:id`. **Destapó** que ninguna mutación de órdenes refrescaba la caché de productos. **El E2E, lanzado tras autorizarlo el usuario, destapó que T5-02 rompió el smoke** (tarjeta de valor renombrada); contra servidores levantados a mano cae por el 429 del límite de peticiones. Fuera: el formulario de venta carga 200 productos; stock bajo y alertas siguen en stock físico. |
| 2026-09-13 | **T5-02** Valor del inventario a coste y margen en los informes — **completada** | **El criterio:** una venta enviada con margen 240 **lo conserva** tras subir el coste del producto de 120 a 190. **Sobre `Stockly_carga`** con 330 000 órdenes sembradas: **ninguna de las cuatro consultas ordena en disco** (47.9 / 74.3 / 149.6 / 187.3 ms). En la app levantada, por la interfaz: venta de 2 × 20 con coste 4 → **margen 32, 80 %**. `verify` backend **517/517**, sentencias **93.8 %**, `auditoria` ❌ por los mismos 10 avisos ajenos; frontend ✅ **564/564 + 1 omitido**, **70.49 %** | **La ficha daba por hecho que `SaleOrder` sabía cuándo se envió, y no:** se añade `shippedAt` con índice, relleno en las ya enviadas con `updatedAt`. `unitCost` congelado al enviar, leído de la fila bloqueada. Ventana de 30 días, como la rotación; margen potencial solo sobre productos con coste; lo vendido sin coste queda fuera y se informa, no suma como cero. **Dos errores propios, vistos midiendo:** las consultas de margen se esperaban después de las demás (+190 ms al dashboard) y la siembra de carga metía las 660 000 líneas en un solo producto por un `random()` evaluado una vez. **Destapó** que el desglose de stock por categoría manda «Sin categoría» en español a la interfaz inglesa; el de margen manda `null`. Pendiente: ventas en `load/sembrar.js` y repetir `carga:ejecutar`. |
...
```bash
grep -c '^- \[x\] \*\*\[T' docs/ROADMAP.md    # 118
grep -c '^- \[ \] \*\*\[T' docs/ROADMAP.md    # 11 (el Tier 5, abierto el 2026-09-13)
```
...
|---|---|---|---|
| Tests backend | 198/198 ✅ | **547/547** ✅ *(2026-09-13, T5-04)* | mantener en verde |
| Cobertura backend (sentencias) | 86.92 % | **94.23 %** ✅ *(suelo en 85 %, T2-22)* | ≥ 88 % |
| Tests frontend | 181/181 ✅ | **582/582** ✅ *(+1 omitido: la frescura del contrato sin el repo hermano; 2026-09-13)* | mantener en verde |
| Cobertura frontend (sentencias) | 19.88 % | **72.27 %** ✅ *(suelo subido a 45 % con T4-01)* | ≥ 45 % — **alcanzado** |
...
| Métrica | 2026-07-15 (dev) | **Medido en T4-09** (producción, 2026-08-11) | Objetivo |
|---|---|---|---|
| Lighthouse — Accesibilidad | 96 *(fallo: contraste de color, ya corregido)* | **100** ✅ *(login 93, productos 94, movimientos 95 antes de los arreglos)* | ≥ 95 |
| Lighthouse — Buenas prácticas | 100 | **100** ✅ | mantener |
| Lighthouse — SEO | 82 *(faltaban `lang` y meta description, ya corregidos)* | **63** — *y está bien*: penaliza el bloqueo de indexación que decidió T3-12 | no aplica a una aplicación privada |
| LCP (dashboard) | 556 ms | **420 ms** ✅ *(2 ms de TTFB; el resto, arranque de la SPA)* | < 2.5 s |
| CLS | 0.02 | **0.02** ✅ | < 0.1 |
| Errores de consola | ninguno | **ninguno** ✅ | ninguno |

*El detalle, con lo que **no** se comprobó, en [accesibilidad.md](accesibilidad.md).*
