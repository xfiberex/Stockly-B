...
import { uploadToCloudinary, deleteFromCloudinary } from "../../common/middlewares/upload.middleware";
import type { CreateProductDto, UpdateProductDto, ProductQuery } from "./product.types";
  ...
  next: NextFunction
    ...
    try {
        ...
        const where = {
          ...
          ...(req.query.search && {
            name: { contains: req.query.search, mode: "insensitive" as const },
          }),
...
export async function createProduct(
    ...
    try {
        ...
        if (req.file) {
            const uploaded = await uploadToCloudinary(req.file.buffer, "stockly/products");
...
export async function updateProduct(
    ...
    try {
        ...
        const product = await prisma.product.update({
            ...
            data: { ...req.body, imageUrl, imagePublicId },
        });
...
export async function restoreProduct(
  ...
  try {
    ...
    if (existing.isActive) {
      res.status(400).json({ success: false, message: "El producto ya está activo" });
      return;
    }
